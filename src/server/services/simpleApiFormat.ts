...
export interface SimpleField {
  name: string;
  /** "string" (default) | "integer" | "number" | "boolean" | "object" | "array" */
  type?: string;
  required?: boolean;
  description?: string;
  /** Optional format hint, e.g. "date", "date-time", "email", or a custom pattern like "YYYY-MM-DD" — passed through as-is. */
  format?: string;
  /** Optional example value shown in Swagger UI, e.g. 1, "jdoe@example.com". */
  example?: any;
  /** For type: "object" — the nested structure's own fields. */
  fields?: SimpleField[];
  /** For type: "array" of objects — each item's fields (an ABAP table of a structure). */
  items?: SimpleField[];
  /** For type: "array" of scalars — the item type, e.g. "string" (an ABAP table of a single field). Defaults to "string". */
  itemType?: string;
  /** Escape hatch: a raw JSON-Schema object, used as-is instead of being built from the fields above. */
  schema?: Record<string, any>;
}

export interface SimpleResponse {
  /** HTTP status code, e.g. 200, 201, 400, 404. Defaults to 200. */
...
function fieldToSchema(f: SimpleField): Record<string, any> {
...
function buildObjectSchema(fields: SimpleField[]): Record<string, any> {
...
function expandParameter(p: any, path: string): Record<string, any> {
  ...
  if (!p?.schema && schema && typeof schema === 'object') delete schema.description;
  return {
    name: p?.name,
    in: inferredIn,
...
function expandRequestBody(endpoint: any): Record<string, any> | undefined {
  // Already fully-shaped — pass through as-is.
  if (endpoint.requestBody) return endpoint.requestBody;

  const fields: SimpleField[] | undefined = endpoint.requestFields;
...
function expandOneResponse(r: SimpleResponse): Record<string, any> {
...
function expandResponses(endpoint: any): Record<string, any> {
  ...
  const fields: SimpleField[] | undefined = endpoint.responseFields;
  return {
    200: {
...
function expandEndpoint(e: any): Record<string, any> {
  ...
  return {
    ...
    tags: e?.tags,
    parameters: Array.isArray(e?.parameters) ? e.parameters.map((p: any) => expandParameter(p, path)) : undefined,
 ...
 */
export function expandSimpleApi(payload: any): any {
  if (!payload || typeof payload !== 'object') return payload;
  if (!Array.isArray(payload.endpoints)) return payload;
